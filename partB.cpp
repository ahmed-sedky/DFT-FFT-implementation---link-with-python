...
void dft(complex<double> inp_sig[],complex<double> Output_Signal[],double Size) // vector of complex numbers of type double  // s :represents our signal
{
	
	complex<double>int_sum;
	
	// we reserve a block of memory beacause we konw exactly how many data point swe are going to we are going to put in this array # it enhance the perfomance i fwe pre allocate the memory we will use
	// we will loop through each value of samples
	for (int k = 0; k < Size; k++)
	{
		...
		for (int n = 0; n < Size; n++)
		{
			...
			double realPartCos = cos(((2 * M_PI) / Size) * z);
			double ImgPartSin = sin(((2 * M_PI) / Size) * z);
